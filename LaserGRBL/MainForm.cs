...
namespace LaserGRBL
{
    public partial class MainForm : Form
    {
        ...
        private void ManageMessage()
        {
            try
            {
                ...
                if (ToolBarMessage != null && ToolBarMessage.Title != null && ToolBarMessage.Content != null)
                {
                    ...
                    TTLinkToNews.LinkBehavior = LinkBehavior.HoverUnderline;
                }
            }
        ...
        {
            if (Core.InProgram && MessageBox.Show(Strings.ExitAnyway, Strings.WarnMessageBoxHeader, MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) != DialogResult.Yes)
                e.Cancel = true;
        ...
        private void TimerUpdate()
        {
            ...
            PbBuffer.ToolTipText = $"Buffer: {Core.UsedBuffer}/{Core.BufferSize} Livre:{Core.FreeBuffer}";
        ...
        internal virtual ContextMenuStrip GetOvMenu()
        {
        ...
        {
            if (!(Core.InProgram && MessageBox.Show(Strings.DisconnectAnyway, Strings.WarnMessageBoxHeader, MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) != DialogResult.Yes))
                Core.CloseCom(true);
        }
        ...
        private void MainForm_DragDrop(object sender, DragEventArgs e)
        {
            if (droppedFile == null)
            {
                ...
                if (files.Length == 1)
                {
                    ...
                    {
                        dropDispatcherTimer = new DispatcherTimer();
                        dropDispatcherTimer.Interval = TimeSpan.FromSeconds(0.5);
                        dropDispatcherTimer.Tick += new EventHandler(DropDispatcherTimer_Tick);
                    }

                    dropDispatcherTimer.Start();
                }
            }
        }
        ...
        void DropDispatcherTimer_Tick(object sender, EventArgs e)
        {
            if (droppedFile != null)
            {
                Core.OpenFile(this, droppedFile);
                droppedFile = null;
